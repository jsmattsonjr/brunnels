...
@pytest.mark.parametrize(
    "tags, keep_polygons, expected_reason",
    [
        # Bicycle=no
        ({"bicycle": "no"}, False, FilterReason.BICYCLE_NO),
        ({"bicycle": "no"}, True, FilterReason.BICYCLE_NO),
        # Waterway
        ({"waterway": "river"}, False, FilterReason.WATERWAY),
        ({"waterway": "stream"}, True, FilterReason.WATERWAY),
        # Railway (not abandoned)
        ({"railway": "rail"}, False, FilterReason.RAILWAY),
        ({"railway": "tram"}, True, FilterReason.RAILWAY),
        # Polygon (closed way)
        ({}, False, FilterReason.NONE),  # Default, non-polygon
        ({"highway": "footway"}, False, FilterReason.NONE), # Non-polygon with other tags
    ],
)
def test_should_filter_various_reasons(
    tags: dict[str, str], keep_polygons: bool, expected_reason: FilterReason
):
    # For polygon tests, nodes are handled separately if needed
    # Default nodes for non-polygon cases
    nodes = [1, 2]
    brunnel_way = create_brunnel_way(1, nodes, tags)
    assert BrunnelWay.should_filter(brunnel_way.metadata, keep_polygons) == expected_reason

# Specific tests for polygon filtering
def test_should_filter_polygon_filter_out():
    nodes_polygon = [1, 2, 3, 1] # Closed way
    brunnel_way_polygon = create_brunnel_way(1, nodes_polygon, {})
    assert BrunnelWay.should_filter(brunnel_way_polygon.metadata, keep_polygons=False) == FilterReason.POLYGON

def test_should_filter_polygon_keep():
    nodes_polygon = [1, 2, 3, 1] # Closed way
    brunnel_way_polygon = create_brunnel_way(1, nodes_polygon, {})
    assert BrunnelWay.should_filter(brunnel_way_polygon.metadata, keep_polygons=True) == FilterReason.NONE
...
@pytest.mark.parametrize(
    ...
    [
        # No relevant tags
        ({}, False),
        ({"name": "Test Bridge"}, True),
        # Bicycle=yes/designated/etc.
        ({"bicycle": "yes"}, False),
        ({"bicycle": "designated"}, True),
        # Cycleway
        ({"highway": "cycleway"}, False),
        ({"cycleway": "track"}, True), # Other cycleway tags also imply bicycle use
        # Abandoned railway
        ({"railway": "abandoned"}, False),
        ({"railway": "abandoned", "bicycle": "dismount"}, True), # bicycle=dismount is not "no"
        ...
        ({"highway": "road", "bicycle": "yes", "waterway": "canal"}, False), # waterway is ignored due to bicycle=yes
        ({"railway": "preserved", "bicycle": "permissive"}, True), # railway ignored due to bicycle=permissive
    ],
)
def test_should_not_filter(tags: dict[str, str], keep_polygons: bool):
    brunnel_way = create_brunnel_way(1, [1, 2], tags)
    assert BrunnelWay.should_filter(brunnel_way.metadata, keep_polygons) == FilterReason.NONE
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"bridge": "yes", "tunnel": "yes"}, BrunnelType.TUNNEL),  # Tunnel tag takes precedence
        ...
        ({"man_made": "bridge"}, BrunnelType.BRIDGE), # Common alternative tag for bridge
...
def test_is_contained_by_true_complex_route(complex_route_geometry: Polygon):
    ...
    brunnel_coords_horizontal = [Position(1.5, 0.25), Position(2.5, 0.75)] # Horizontal part
...
def test_is_contained_by_false_complex_route(complex_route_geometry: Polygon):
    ...
    brunnel_coords = [Position(0.5, 2.5), Position(1.5, 2.5)] # Crosses from inside to outside top part
    brunnel = MockBrunnel(BrunnelType.BRIDGE, brunnel_coords)
...
def test_is_contained_by_empty_brunnel_coords(simple_route_geometry: Polygon):
    ...
    assert brunnel.is_contained_by(simple_route_geometry) is False # No linestring to check
...
def test_calculate_route_span_brunnel_along_route_segment(
    ...
    route_close = create_mock_route([Position(0,0), Position(0.001, 0), Position(0.002, 0)])
...
def test_calculate_route_span_brunnel_offset_from_route(
    ...
    brunnel_coords = [Position(1, 0.1), Position(2, 0.1)] # y is latitude, x is longitude
    brunnel = MockBrunnel(BrunnelType.BRIDGE, brunnel_coords)
    ...
    route_points = [Position(0, 0), Position(0, 0.01), Position(0, 0.02)] # Route along equator
    ...
    assert route_span.length_km == pytest.approx(simple_cum_dist[2] - simple_cum_dist[1])
...
def test_calculate_route_span_brunnel_partially_spans_segment(
    ...
    test_route = create_mock_route([Position(0,0), Position(0,1), Position(0,2), Position(0,3), Position(0,4), Position(0,5)])
    ...
    brunnel_coords = [Position(0, 1.5), Position(0, 2.5)] # Brunnel from (0,1.5) to (0,2.5)
    brunnel = MockBrunnel(BrunnelType.BRIDGE, brunnel_coords)
    ...
    assert route_span.length_km == pytest.approx(expected_end_dist - expected_start_dist)
...
def test_calculate_route_span_empty_brunnel_coords(
    ...
    brunnel = MockBrunnel(BrunnelType.BRIDGE, [])
    route_span = brunnel.calculate_route_span(linear_route, linear_route_cumulative_distances)
...
def test_calculate_route_span_single_point_brunnel(
    ...
    assert route_span.end_distance_km == pytest.approx(expected_dist) # Start and end are the same for a point
...
def horizontal_route() -> Route:
    ...
    return create_mock_route([Position(0, 0), Position(0, 1), Position(0, 2)]) # Eastward: lon changes, lat is 0

@pytest.fixture
def vertical_route() -> Route:
    # Route: P(0,0) -> P(0,1) -> P(0,2) (Northward if Position(lat,lon))
    return create_mock_route([Position(0, 0), Position(1, 0), Position(2, 0)]) # Northward: lat changes, lon is 0
...
def test_is_aligned_with_route_perfectly_aligned_horizontal(horizontal_route: Route):
    ...
    brunnel_coords_east = [Position(0, 0.5), Position(0, 1.5)] # Brunnel lon from 0.5 to 1.5, lat = 0. Bearing East (90)
...
def test_is_aligned_with_route_perfectly_aligned_vertical(vertical_route: Route):
    ...
    brunnel_coords = [Position(0.5, 0), Position(1.5, 0)] # Brunnel lat from 0.5 to 1.5, lon = 0. Bearing North (0)
    brunnel = MockBrunnel(BrunnelType.BRIDGE, brunnel_coords)
...
def test_is_aligned_with_route_perpendicular(horizontal_route: Route):
    ...
    brunnel_coords = [Position(0.5, 1.0), Position(1.5, 1.0)] # lat from 0.5 to 1.5, lon = 1.0
                                                            # Brunnel is North (0/360 deg)
                                                            # Route is East (90 deg)
                                                            # Difference is 90 deg.
    brunnel = MockBrunnel(BrunnelType.BRIDGE, brunnel_coords)
    assert brunnel.is_aligned_with_route(horizontal_route, tolerance_degrees=10) is False
...
def test_is_aligned_with_route_within_tolerance(horizontal_route: Route):
    ...
    brunnel = MockBrunnel(BrunnelType.BRIDGE, brunnel_coords)
    assert brunnel.is_aligned_with_route(horizontal_route, tolerance_degrees=10) is True
    assert brunnel.is_aligned_with_route(horizontal_route, tolerance_degrees=5) is False # Outside 5 deg tolerance
...
def test_is_aligned_with_route_short_brunnel_or_route():
    ...
    brunnel = MockBrunnel(BrunnelType.BRIDGE, brunnel_coords)
    assert brunnel.is_aligned_with_route(route_single_point, 10) is False # Route too short

    brunnel_single_point = MockBrunnel(BrunnelType.BRIDGE, [Position(0,0)])
    # horizontal_route is P(0,0), P(0,1), P(0,2)
    assert brunnel_single_point.is_aligned_with_route(horizontal_route, 10) is False # Brunnel too short
...
def test_detect_adjacent_groups_no_groups():
    brunnels = [
        ...
        create_test_way(2, [30, 40], start_km=2, end_km=3), # B2 (no shared node with B1)
...
def test_detect_adjacent_groups_one_group_two_brunnels():
    brunnels = [
        create_test_way(1, [10, 20], start_km=0, end_km=1), # B1
        create_test_way(2, [20, 30], start_km=1, end_km=2), # B2 (shares node 20 with B1)
...
def test_detect_adjacent_groups_mixed_types_no_grouping_across_types():
    brunnels = [
        create_test_way(1, [10, 20], brunnel_type=BrunnelType.BRIDGE, start_km=0, end_km=1), # Bridge 1
        create_test_way(2, [20, 30], brunnel_type=BrunnelType.TUNNEL, start_km=1, end_km=2), # Tunnel 1 (shares node but different type)
        create_test_way(3, [30, 40], brunnel_type=BrunnelType.TUNNEL, start_km=2, end_km=3), # Tunnel 2
    ]
...
def test_detect_adjacent_groups_respects_filters_and_containment():
    brunnels = [
        create_test_way(1, [10, 20], start_km=0, end_km=1, contained=True, filter_reason=FilterReason.NONE),       # 0: Valid B1
        create_test_way(2, [20, 30], start_km=1, end_km=2, contained=False, filter_reason=FilterReason.NONE),      # 1: Not contained
        create_test_way(3, [30, 40], start_km=2, end_km=3, contained=True, filter_reason=FilterReason.RAILWAY),    # 2: Filtered out
        create_test_way(4, [40, 50], start_km=3, end_km=4, contained=True, filter_reason=FilterReason.NONE),       # 3: Valid B2 (no connection to B1)
        create_test_way(5, [50, 60], start_km=4, end_km=5, contained=True, filter_reason=FilterReason.NONE),       # 4: Valid B3 (connects to B2)
        create_test_way(6, [70, 80], start_km=6, end_km=7, contained=True, filter_reason=FilterReason.NONE, brunnel_type=BrunnelType.TUNNEL), # 5: Valid T1
        create_test_way(7, [80, 90], start_km=7, end_km=8, contained=True, filter_reason=FilterReason.NONE, brunnel_type=BrunnelType.TUNNEL), # 6: Valid T2 (connects to T1)
    ]
    ...
    detected_groups = sorted(CompoundBrunnelWay.detect_adjacent_groups(brunnels), key=lambda x: x[0])
...
def test_detect_adjacent_groups_order_by_start_distance():
    ...
    brunnels = [
        create_test_way(1, [20, 30], start_km=1, end_km=2), # Index 0, B2 (connects to B1)
        create_test_way(2, [10, 20], start_km=0, end_km=1), # Index 1, B1
        create_test_way(3, [30, 40], start_km=2, end_km=3), # Index 2, B3 (connects to B2)
    ]
...
def test_create_from_brunnels_one_compound_formed():
    brunnels = [
        create_test_way(1, [10, 20], start_km=0, end_km=1, brunnel_type=BrunnelType.BRIDGE), # B1
        create_test_way(2, [20, 30], start_km=1, end_km=2, brunnel_type=BrunnelType.BRIDGE), # B2 (adj to B1)
        create_test_way(3, [40, 50], start_km=3, end_km=4, brunnel_type=BrunnelType.BRIDGE), # B3
    ]
    result = CompoundBrunnelWay.create_from_brunnels(brunnels)
    assert len(result) == 2 # One compound + B3

    compound_brunnel = next((b for b in result if isinstance(b, CompoundBrunnelWay)), None)
    ...
    assert compound_brunnel.filter_reason == FilterReason.NONE # Set by create_from_brunnels

    # Check route span of compound
    assert compound_brunnel.route_span is not None
    assert compound_brunnel.route_span.start_distance_km == pytest.approx(0.0) # min of B1, B2 start_km
    assert compound_brunnel.route_span.end_distance_km == pytest.approx(2.0)   # max of B1, B2 end_km

    remaining_brunnel = next((b for b in result if isinstance(b, BrunnelWay) and not isinstance(b, CompoundBrunnelWay)), None)
...
def test_create_from_brunnels_multiple_compounds_and_individuals():
    brunnels = [
        create_test_way(1, [10, 20], start_km=0, end_km=1, brunnel_type=BrunnelType.BRIDGE),  # C1-B1
        create_test_way(2, [20, 30], start_km=1, end_km=2, brunnel_type=BrunnelType.BRIDGE),  # C1-B2
        create_test_way(3, [100, 110], start_km=2.5, end_km=3, brunnel_type=BrunnelType.BRIDGE), # Individual Bridge
        create_test_way(4, [40, 50], start_km=3, end_km=4, brunnel_type=BrunnelType.TUNNEL),  # C2-T1
        create_test_way(5, [50, 60], start_km=4, end_km=5, brunnel_type=BrunnelType.TUNNEL),  # C2-T2
        create_test_way(6, [60, 70], start_km=5, end_km=6, brunnel_type=BrunnelType.TUNNEL),  # C2-T3
        create_test_way(7, [200, 210], start_km=7, end_km=8, brunnel_type=BrunnelType.BRIDGE), # Individual Bridge 2
    ]
    result = CompoundBrunnelWay.create_from_brunnels(brunnels)
    ...
    individual_brunnels = [b for b in result if isinstance(b, BrunnelWay) and not isinstance(b, CompoundBrunnelWay)]
...
def test_create_from_brunnels_handles_detection_failure_gracefully():
    ...
    brunnels = [
        create_test_way(1, [10, 20], start_km=0, end_km=1),
        create_test_way(2, [20, 30], start_km=1, end_km=2, contained=False) # No route_span because not contained
    ]
    result = CompoundBrunnelWay.create_from_brunnels(brunnels)
