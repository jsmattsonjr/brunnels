...
from brunnels.route import Route  # Added import
...
def test_haversine_distance_short_distance():
    ...
    assert haversine_distance(pos1, pos2) == pytest.approx(
        expected_distance_km, abs=0.0001
    )
...
def test_point_off_segment_closest_to_middle():
    ...
    assert closest_point.longitude == pytest.approx(
        expected_closest_on_segment.longitude
    )
...
def test_zero_length_segment():
    ...
    assert distance == pytest.approx(
        157.2533733278164, abs=0.0001
    )  # Adjusted to actual output
    assert t == 0.0
    assert closest_point.latitude == pytest.approx(seg_start.latitude)
    assert closest_point.longitude == pytest.approx(seg_start.longitude)
...
def test_point_far_and_perpendicular_to_segment_midpoint():
    ...
    assert distance == pytest.approx(
        111.19492664455875, abs=0.0001
    )  # Adjusted to actual output
    assert t == pytest.approx(0.5)
    assert closest_point.latitude == pytest.approx(
        expected_closest_on_segment.latitude, abs=1e-3
    )
    assert closest_point.longitude == pytest.approx(
        expected_closest_on_segment.longitude, abs=1e-3
    )
...
def test_find_closest_point_on_route_point_on_segment():
    ...
    route.calculate_distances()  # Ensure distances are calculated

    point = Position(latitude=0.0, longitude=0.5)  # Point on the first segment

    dist_km, closest_pos = find_closest_point_on_route(point, route)

    expected_dist_on_segment = haversine_distance(route.coordinate_list[0], point)
...
def test_find_closest_point_on_route_point_is_vertex():
    ...
    assert dist_km == pytest.approx(route.trackpoints[1]["track_distance"])
    assert closest_pos.latitude == pytest.approx(point.latitude)
    assert closest_pos.longitude == pytest.approx(point.longitude)
...
def test_find_closest_point_on_route_point_off_route():
    ...
    expected_closest_on_segment = Position(latitude=0.0, longitude=0.5)
    expected_cumulative_dist = haversine_distance(
        route.coordinate_list[0], expected_closest_on_segment
    )
...
def test_find_closest_point_on_route_complex_case():
    ...
    for i in range(len(route.coordinate_list) - 1):
        seg_start = route.coordinate_list[i]
        seg_end = route.coordinate_list[i + 1]
        ...
        if perp_dist_check == pytest.approx(0.0, abs=1e-3):
            if (
                t_param_check >= -1e-4 and t_param_check <= 1.0 + 1e-4
            ):  # t_param slightly outside [0,1] is ok if closest_pos *is* an endpoint
                # Check if the proj_point_check is indeed the same as closest_pos
                if closest_pos.latitude == pytest.approx(
                    proj_point_check.latitude, abs=1e-6
                ) and closest_pos.longitude == pytest.approx(
                    proj_point_check.longitude, abs=1e-6
                ):
                    ...
                    recalculated_cumulative_dist_for_closest_pos = (
                        route.trackpoints[i]["track_distance"] + dist_along_segment
                    )
                    break

    assert (
        found_on_segment
    ), "Returned closest_pos does not appear to lie on any segment."
    assert dist_km == pytest.approx(
        recalculated_cumulative_dist_for_closest_pos, abs=0.001
...
def test_calculate_bearing_east():
    ...
    assert calculate_bearing(start, end) == pytest.approx(
        90.0, abs=0.1
    )  # abs allows for slight deviation
...
def test_calculate_bearing_poles():
    ...
    assert calculate_bearing(north_pole, point_south_of_np) == pytest.approx(
        180.0
    )  # Due South

    # To North Pole from some point
    point_near_np = Position(latitude=89.0, longitude=45.0)
    assert calculate_bearing(point_near_np, north_pole) == pytest.approx(
        0.0
    )  # Due North
    ...
    assert calculate_bearing(south_pole, point_north_of_sp) == pytest.approx(
        0.0
    )  # Due North

    # To South Pole from some point
    point_near_sp = Position(latitude=-89.0, longitude=120.0)
    assert calculate_bearing(point_near_sp, south_pole) == pytest.approx(
        180.0
    )  # Due South
...
def test_calculate_bearing_international_date_line_crossing():
    ...
    )  # Crosses date line, effectively 181 or -179
...
def test_bearings_aligned_opposite_direction_perfect():
    assert (
        bearings_aligned(45.0, 225.0, tolerance_degrees=1.0) == True
    )  # 45 + 180 = 225
...
def test_bearings_aligned_opposite_at_tolerance_limit():
    ...
    assert (
        bearings_aligned(45.0, 223.9, tolerance_degrees=1.0) == False
    )  # Just outside 180-1 limit
...
def test_bearings_aligned_wraparound_zero_360():
    assert (
        bearings_aligned(10.0, 350.0, tolerance_degrees=20.0) == True
    )  # 10 is like 370, 370-350=20
    assert (
        bearings_aligned(350.0, 10.0, tolerance_degrees=20.0) == True
    )  # 350 vs 10 (or 370)
    ...
    assert (
        bearings_aligned(10.0, 170.0, tolerance_degrees=20.0) == True
    )  # (10 vs 170) -> diff 160. |160-180|=20. True.
...
def test_bearings_aligned_large_tolerance():
    ...
    assert (
        bearings_aligned(0.0, 180.0, tolerance_degrees=90.0) == True
    )  # 0 vs 180, diff 180. |180-180|=0 <= 90. True.
    assert (
        bearings_aligned(0.0, 270.0, tolerance_degrees=90.0) == True
    )  # 0 vs 270, diff 270. min(270, 360-270=90)=90. 90<=90. True.
