...
class TestBearingProperties:
    ...
    @given(valid_position, valid_position)
    def test_opposite_bearing_roughly_opposite(self, pos1, pos2):
        """Bearing from A to B should be roughly opposite to B to A (spherical geometry)."""
        ...
        # On a sphere, opposite bearings are roughly 180° apart but not exactly
        # due to curvature. Use generous tolerance for spherical geometry.
        assert abs(diff - 180) < 10.0  # Within 10° of opposite

        # Also test they're generally opposite (not same direction)
        assert diff > 90.0  # At least 90° different
...
class TestBoundingBoxProperties:
    ...
    @given(
        st.lists(valid_position, min_size=1, max_size=100), st.floats(0, 10)
    )  # Buffer in km
...
class TestCoordinateCombinerProperties:
    ...
    @given(
        st.lists(
            st.lists(valid_position, min_size=2, max_size=10), min_size=2, max_size=5
        )
    )
    def test_coordinate_combining_preserves_total_length(self, component_coords):
        ...
        try:
            ...
            for i in range(1, len(combined_coords)):
                combined_length += haversine_distance(
                    combined_coords[i - 1], combined_coords[i]
                )

            # Should be approximately equal (within 1% tolerance for coordinate precision)
            assert (
                abs(combined_length - total_individual_length)
                / max(total_individual_length, 1e-6)
                < 0.01
            )
