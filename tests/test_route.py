...
'''
    ...
    if not points_data: # Empty GPX
        pass
    elif not isinstance(points_data[0][0], list): # Single track, multiple segments implicitly
    ...
    for track_segments_data in points_data:
        ...
        for segment_points in track_segments_data:
            ...
            for lat, lon, ele in segment_points:
                if ele is not None:
                    points_xml += f'<trkpt lat="{lat}" lon="{lon}"><ele>{ele}</ele></trkpt>\n'
                else:
...
def test_from_gpx_valid_single_track_single_segment():
    gpx_str = gpx_content([
        [(40.7128, -74.0060, 10.0), (40.7580, -73.9855, 12.0)]
    ])
...
def test_from_gpx_valid_multi_track():
    gpx_str = gpx_content([
        ...
        [[(40.7580, -73.9855, 12.0)], [(40.7589, -73.9845, 13.0)]] # Track 2, Segment 1 & 2
    ])
    route = Route.from_gpx(StringIO(gpx_str))
    assert len(route.positions) == 3
    assert route.positions[0] == Position(40.7128, -74.0060, 10.0)
...
def test_from_gpx_point_without_elevation():
    gpx_str = gpx_content([
        [(40.7128, -74.0060, None), (40.7580, -73.9855, 12.0)]
    ])
    route = Route.from_gpx(StringIO(gpx_str))
    assert len(route.positions) == 2
...
def test_from_gpx_invalid_xml():
    ...
    with pytest.raises(GPXException): # gpxpy should raise GPXException for parsing errors
        Route.from_gpx(StringIO(gpx_str))
...
def test_validate_route_near_north_pole():
    ...
    with pytest.raises(RouteValidationError) as excinfo:
        Route.from_gpx(StringIO(gpx_str))
    assert "pole" in str(excinfo.value).lower() or "latitude" in str(excinfo.value).lower()
...
def test_get_bbox_single_point_route_default_buffer():
    ...
    # 1 deg lat ~ 111 km. Buffer is 1km by default.
    # So lat_buffer is 1/111.0
    # lon_buffer depends on latitude. At lat 0, it's also 1/111.0
    ...
    lon_buffer_deg = buffer_km / (111.0 * 1.0) # cos(radians(0)) = 1
    ...
    bbox = route.get_bbox() # Default buffer_km = 1.0
...
def test_get_bbox_single_point_route_custom_buffer():
    ...
    lon_buffer_deg = buffer_km / (111.0 * abs(math.cos(math.radians(pos.latitude))))
    ...
    bbox = route.get_bbox(buffer_km=buffer_km)
    assert bbox == pytest.approx(expected_bbox)
...
def test_get_bbox_multi_point_route_no_buffer_implicitly_zero():
    ...
    # The _calculate_bbox method is what takes buffer_km. get_bbox uses it.
    ...
    # With default buffer_km = 1.0
    buffer_km = 1.0
    avg_lat = 0.5 # (0+1)/2
    lat_buffer_deg = buffer_km / 111.0
    lon_buffer_deg = buffer_km / (111.0 * abs(math.cos(math.radians(avg_lat))))
...
def test_get_bbox_multi_point_route_larger_buffer():
    positions = [
        ...
        Position(latitude=12, longitude=13, elevation=0) # min_lat=10, max_lat=12, min_lon=10, max_lon=13
    ]
    route = Route(positions=positions)

    buffer_km = 10.0 # Larger buffer
...
def test_get_bbox_memoization():
    ...
    route = Route(positions=positions)
    bbox1 = route.get_bbox(buffer_km=1.0)
    # Access internal attributes for testing memoization (with caution)
    assert route._bbox is not None
    assert route._bbox_buffer_km == 1.0

    bbox2 = route.get_bbox(buffer_km=1.0) # Same buffer, should be memoized
    assert bbox1 is bbox2 # Should be the exact same object

    bbox3 = route.get_bbox(buffer_km=2.0) # Different buffer, should recompute
    assert bbox3 is not bbox1 # Should be a new object
    assert route._bbox_buffer_km == 2.0
    ...
    bbox4 = route.get_bbox(buffer_km=1.0)
    ...
    # The current implementation will recompute if buffer_km changes from the stored _bbox_buffer_km
    # So bbox4 will be a new tuple object, but its values will be same as bbox1.
    assert bbox4 == bbox1
    assert bbox4 is not bbox1 # It recomputes because _bbox_buffer_km was 2.0
    assert route._bbox_buffer_km == 1.0
...
def test_get_cumulative_distances_multi_points():
    ...
    positions = [
        ...
        Position(1, 0, 0),  # Point C (approx 111km from A/B)
        Position(1, 1, 0)   # Point D
    ]
    route = Route(positions=positions)

    dist_ab = 0.0 # haversine_distance(positions[0], positions[1])
    dist_bc = haversine_distance(positions[1], positions[2]) # Approx 111.195 km
    dist_cd = haversine_distance(positions[2], positions[3]) # Approx 111.035 km at lat 1
                                                              # (cos(radians(1)) * 111.195) - more complex actually
    ...
    # P0: (0,0)
    # P1: (0,0) -> dist = 0
    # P2: (1,0) -> dist from P1 = haversine( (0,0), (1,0) ) approx 111.195 km
    ...
    d0 = 0.0
    d1 = d0 + haversine_distance(positions_simple[0], positions_simple[1]) # Should be 0
    d2 = d1 + haversine_distance(positions_simple[1], positions_simple[2])
    d3 = d2 + haversine_distance(positions_simple[2], positions_simple[3]) # Should be 0
...
def test_route_spans_overlap_adjacent_touching():
    # Spans are considered overlapping if start_distance_km <= end_distance_km and vice-versa
    span1 = RouteSpan(start_distance_km=0.0, end_distance_km=10.0)
